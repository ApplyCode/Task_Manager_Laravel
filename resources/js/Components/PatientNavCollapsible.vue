...
const props = defineProps(['patients', 'slimNavigation', 'isMobile']);

const isActive = ref(localStorage.getItem('navPatientsCollapse') === 'true');

const toggleCollapse = () => {
    isActive.value = !isActive.value;
    localStorage.setItem('navPatientsCollapse', isActive.value);
};
</script>
        ...
        >
            <div v-if="isActive" class="pl-3 pr-4">
                <TransitionGroup name="list" tag="ul">
                    <li v-for="(patient, index) in props.patients" :key="index">
                        <ResponsiveNavLink :href="route('patients.show', patient._id)" :active="route().current('patients.show', patient._id)" class="flex items-center" v-tooltip="'Patient ' + patient.name">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 15 15" :class="{
                                 'transition-all': true,
                                  'mr-2': !slimNavigation,
                                 '-mx-4': slimNavigation,
                            }"><path :fill="stringToColour(patient.name)" d="M9.875 7.5a2.375 2.375 0 1 1-4.75 0a2.375 2.375 0 0 1 4.75 0Z"/></svg>
                            <span :class="{
                                'hidden': slimNavigation,
                                'transition-all': true
                            }"
                            :style="{ 'overflow': 'hidden', 'text-overflow': 'ellipsis', 'white-space': 'nowrap' }"
                            >{{ patient.name.length > 20 ? patient.name.slice(0, 20) + '...': patient.name }}</span>
                            <div class="flex grow justify-end">
                                <div :class="{'inline-block rounded-md h-5 w-5 text-white flex items-center justify-center text-xs font-bold uppercase': true,
                                    'hidden': slimNavigation,
                                    'transition-all': true
                                }" :style="{ backgroundColor: stringToColour(patient.user.name) }">
                    ...
                    <li key="last">
                        <ResponsiveNavLink :href="route('patients')" :active="route().current('patients')" class="flex items-center" v-tooltip="'All the patients'">
