<!--
    COMPONENT: Show
    DESCRIPTION: This component displays the details of a patient, including its name, address, and appointments. It also allows the user to delete multiple appointments at once.
    PROPS:
    ...
    EVENTS:
        - appoitmentCheckbox: An event that is emitted when a appoitment checkbox is clicked.
...
const allAppointments = [...patient.value.appointments].flat().filter((appoitment, index, self) => index === self.findIndex((t) => t._id === appoitment._id));

function displayMenuFunc() {
    return selectedAppointments?.value.some((appoitment) => allAppointments.some((t) => t._id === appoitment));
};

const displayMenu = ref(displayMenuFunc());

onMounted(() => {
    eventBus.$on('appoitmentCheckbox', (content) => {
        //check if content is in selectedAppointments if not add it if yes remove it
        if (selectedAppointments.value.includes(content)) {
            selectedAppointments.value = selectedAppointments.value.filter((appoitment) => appoitment !== content);
        } else {
        ...
        switch (data.type) {
            case 'update':
                if (patient.value._id === data.patient._id) {
                    ...
                    patient.value.address = data.patient.address;
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Patient {{ patient.name }}</h2>
        </template>
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden">
            <div class="flex bg-gray-50 dark:bg-gray-950">
                ...
                >
                    ...
                    <div class="relative">
                        <Dropdown align="appoitmentContext" width="48">
                            <template #trigger>
                                <div class="px-4"
                                    :class="{
